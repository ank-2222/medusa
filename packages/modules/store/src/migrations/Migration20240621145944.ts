...
export class Migration20240621145944 extends Migration {
  async up(): Promise<void> {
    this.addSql(
      'create table if not exists "store_currency" ("id" text not null, "currency_code" text not null, "is_tax_inclusive" boolean not null default true, "is_default" boolean not null default false, "store_id" text null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "store_currency_pkey" primary key ("id"));'
    )
    this.addSql(
